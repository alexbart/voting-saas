...
async function login(req, res) {
  try {
    ...
    return success(res, { token, user: { id: user.id, name: user.name, role: user.role } }, 'Login successful');
  } catch (err) {
    console.error(err);
...
async function register(req, res) {
  try {
    ...
    await registerUser({ name, email, password, role, context_type, external_id });
    await logEvent(null, 'USER_REGISTERED', { email, role, context_type }, req.clientIp);
